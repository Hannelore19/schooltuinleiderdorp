// src/data/news.js
export const news = [
  {
    slug: "open-avond",
    title: "Open avond",
    date: "2026-06-01",
    intro: "Altijd hadden we mooi weer op de Open Avond van de schooltuinen in Leiderdorp. Maar dit keer liet het weer ons in de steek.",
    text: `
      Altijd hadden we mooi weer op de Open Avond van de schooltuinen in Leiderdorp. 
      Maar dit keer liet het weer ons in de steek.

      Toch stonden de eerste leerlingen, ouders, opa’s, oma’s en verdere kennissen 
      om zeven uur al voor het hek. Een bont gezelschap, misschien niet zoveel als 
      andere jaren, maar wel net zo enthousiast.

      De schooltuinen lagen er prachtig bij: groene kropsla en andijvie, aardappels 
      al hoog boven de grond en sperziebonen die langs de bonenstokken omhoog klommen.

      In de grote tent kon je schuilen én iets bijzonders zien: veenmollen. Grote, 
      ondergronds levende insecten met mollenachtige voorpootjes. Best een beetje eng.

      Ondanks de regen was het een geslaagde avond, iets korter dan andere jaren.

      Link naar radio-interview met voorzitter Joop van Huut:
      https://centraalplus.nl/2026/06/01/woensdag-open-avond-bij-schooltuintjes-leiderdorp/
    `,
    photos: [
      "/schooltuinleiderdorp/assets/oa261.jpg",
      "/schooltuinleiderdorp/assets/oa262.jpg",
      "/schooltuinleiderdorp/assets/oa263.jpg",
      "/schooltuinleiderdorp/assets/oa264.jpg",
      "/schooltuinleiderdorp/assets/oa265.jpg",
      "/schooltuinleiderdorp/assets/oa266.jpg"
    ]
  },

  {
    slug: "aardappeloogst",
    title: "Aardappeloogst",
    date: "2026-05-20",
    intro: "De aardappeloogst leverde dit jaar een paar verrassend grote exemplaren op.",
    text: `
      De aardappeloogst leverde dit jaar een paar verrassend grote exemplaren op. 
      De leerlingen waren trots op hun werk en verbaasd over de grootte van sommige aardappels.

      Later komen hier meer foto's bij van het oogsten.
    `,
    photos: [
      "/schooltuinleiderdorp/assets/foto3.jpg"
    ]
  },

  {
    slug: "bijenproject",
    title: "Bijenproject gestart",
    date: "2026-04-15",
    intro: "Het bijenproject is gestart! De leerlingen leren alles over bestuiving en bijen.",
    text: `
      Het bijenproject is gestart! De leerlingen leren alles over bestuiving, bloemen 
      en het belang van bijen voor de natuur. Later komen hier foto's bij van kinderen 
      in imkerpakken.
    `,
    photos: [
      "/schooltuinleiderdorp/assets/zonnebloembij.png",
      "/schooltuinleiderdorp/assets/bijopbloem.png"
    ]
  },

  // --- NIEUWSPAGINA ---
  {
    slug: "opening-seizoen",
    title: "Opening Schooltuinseizoen 2026",
    date: "2026-04-02",
    intro: "Het schooltuinseizoen werd feestelijk geopend door wethouder Herman Romein en leerlingen van Het Bolwerk.",
    text: `
      Op 2 april 2026 werd het schooltuinseizoen officieel geopend door wethouder 
      Herman Romein samen met leerlingen van Het Bolwerk.

      Bekijk hier het prachtige filmpje van streekomroep Centraal:
      https://centraalplus.nl/2026/04/02/kinderen-openen-schooltuinseizoen-in-leiderdorp-met-vieze-handen-en-frisse-lucht/

      De leerlingen kwamen op de fiets naar de schooltuinen. Je kon ze al van ver 
      horen aankomen. Het was nog een hele klus om alle fietsen netjes in het rek 
      te krijgen.

      In de grote tent gaf de wethouder een korte toespraak. Daarna mochten de 
      kinderen zich uitleven bij het paadjes trappen — achter de wethouder aan.

      Daarna begon het echte werk: een tuinbordje maken, mest strooien, aardappels 
      poten en sterrenkers zaaien.
    `,
    photos: [
      "/schooltuinleiderdorp/assets/os261.jpg",
      "/schooltuinleiderdorp/assets/os262.jpg",
      "/schooltuinleiderdorp/assets/os263.jpg",
      "/schooltuinleiderdorp/assets/os264.jpg",
      "/schooltuinleiderdorp/assets/os265.jpg"
    ]
  }
];
